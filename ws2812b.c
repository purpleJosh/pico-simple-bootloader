// Taken directly from https://forums.raspberrypi.com/viewtopic.php?t=322218 with some small modifications
...
{
	for (uint32_t i = LED_DATA_SIZE; i < LED_BYTE_SIZE ; i += LED_DATA_SIZE)
	{
		led_data[i] = g;		/* Green */
		led_data[i + 1] = r;		/* Red */
		led_data[i + 2] = b;		/* Blue */
...
// Commented main code is kept to show
// how the above functions are meant to be used
